package com.ag.agaicodemother.langgraph4j;
...
import com.ag.agaicodemother.langgraph4j.model.QualityResult;
import com.ag.agaicodemother.langgraph4j.node.*;
import com.ag.agaicodemother.langgraph4j.node.concurrent.*;
...
public class CodeGenConcurrentWorkflow {
    ...
    public WorkflowContext executeWorkflow(String originalPrompt) {
        ...
        int stepCounter = 1;
        // 配置并发执行
        // 创建自定义线程池用于并发执行图片收集节点，避免阻塞主流程
        ExecutorService pool = ExecutorBuilder.create()
                .setCorePoolSize(10)           // 核心线程数：保持一定数量的活跃线程以应对常规并发
                .setMaxPoolSize(20)            // 最大线程数：在负载高时扩展线程处理突发请求
                .setWorkQueue(new LinkedBlockingQueue<>(100)) // 任务队列：缓冲待执行任务，防止线程频繁创建销毁
                .setThreadFactory(ThreadFactoryBuilder.create().setNamePrefix("Parallel-Image-Collect").build()) // 命名线程工厂：便于日志追踪和调试
                .build();

        // 配置运行时参数，将自定义线程池绑定到特定的并行节点 "image_plan"
        RunnableConfig runnableConfig = RunnableConfig.builder()
                .addParallelNodeExecutor("image_plan", pool) // 指定当工作流到达 "image_plan" 节点时，使用上述线程池并行执行其子节点
                .build();
