...
public class AppServiceImpl extends ServiceImpl<AppMapper, App>  implements AppService{
    ...
    public String deployApp(Long appId, User loginUser) {
        ...
        if(enumByValue == CodeGenTypeEnum.VUE_PROJECT){
            ...
            ThrowUtils.throwIf(!distDir.exists() || !distDir.isDirectory(), ErrorCode.SYSTEM_ERROR, "Vue 项目构建成功但未生成 dist 目录");
            //将 dist 目录作为部署源
            sourceDir = distDir;
     ...
     * 打包在内存中完成（ByteArrayOutputStream），不产生临时文件，
     * 避免临时文件残留和并发下载时的文件名冲突问题
     *
     * @param appId     应用 ID
     ...
     */
    @Override
    public byte[] downloadApp(Long appId, User loginUser) {
        // 1. 参数校验：应用 id 合法、用户已登录
        ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "应用 id 不能为空");
        ThrowUtils.throwIf(loginUser == null, ErrorCode.PARAMS_ERROR, "用户未登录");
        ...
        // 应用不存在时抛出"数据不存在"异常
        ThrowUtils.throwIf(app == null, ErrorCode.NOT_FOUND_ERROR, "应用不存在");
        // 3. 权限校验：仅创建者本人或管理员可以下载代码
        if (!app.getUserId().equals(loginUser.getId()) && !UserConstant.ADMIN_ROLE.equals(loginUser.getUserRole())) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR, "仅应用创建者或管理员可以下载代码");
        }
        // 4. 定位当前版本目录：tmp/code_output/{codeType}_{appId}/v{currentVersion}
        //    复用 getVersionRootDir 私有方法拼应用根目录（与部署/回退逻辑保持一致）
        //    老数据 currentVersion 为 null 时兜底为 1（与 deployApp 的兜底策略一致）
        int currentVersion = app.getCurrentVersion() == null ? 1 : app.getCurrentVersion();
        String sourceDirPath = getVersionRootDir(app) + File.separator + AppConstant.CODE_VERSION_DIR_PREFIX + currentVersion;
        // 构建 File 对象（仅封装路径，不会在磁盘上创建任何文件/目录）
        File sourceDir = new File(sourceDirPath);
        // 5. 版本目录必须真实存在：不存在说明代码还没生成成功（或生成失败跳空），无法下载
        ThrowUtils.throwIf(!sourceDir.exists() || !sourceDir.isDirectory(),
                ErrorCode.NOT_FOUND_ERROR, "代码尚未生成，请先生成代码后再下载");
        // 6. 将版本目录下的所有文件打包为 zip（内存完成，不落临时文件）
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream();
             ZipOutputStream zos = new ZipOutputStream(baos)) {
            // 递归把目录内容写进 zip；起始相对路径传空串，让文件直接位于 zip 根层级
            // （用户解压后直接得到 index.html、style.css、script.js，不需要再进入一层目录）
            zipDirectory(sourceDir, "", zos);
            // 显式 finish 写入 zip 中央目录记录（必须在 close 之前调用，否则 zip 结构不完整）
            zos.finish();
            // 返回 zip 文件的完整字节数组
            return baos.toByteArray();
        } catch (IOException e) {
            // 打包失败（文件读取/写入异常）包装为业务异常抛出，由全局异常处理器返回错误信息
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "代码打包失败：" + e.getMessage());
        }
    }

    /**
     * 递归把目录中的文件写入 ZipOutputStream（下载功能新增，私有辅助方法）
     *
     * @param dir      要压缩的目录
     * @param basePath 当前文件在 zip 内的相对路径前缀（根目录传空串，文件直接位于 zip 根层级）
     * @param zos      zip 输出流
     * @throws IOException 文件读取或写入失败时抛出
     */
    private void zipDirectory(File dir, String basePath, ZipOutputStream zos) throws IOException {
        // 列出目录下的所有文件和子目录（目录不存在返回 null）
        ...
        // 遍历每个文件/子目录
        for (File file : files) {
            // 计算该文件在 zip 内的条目名：有前缀则拼上前缀，没有则直接用文件名
            String entryName = StrUtil.isBlank(basePath) ? file.getName() : basePath + "/" + file.getName();
            if (file.isDirectory()) {
                // 子目录：递归继续压缩（版本目录理论上无嵌套子目录，保留递归以兼容未来多层级结构）
                zipDirectory(file, entryName, zos);
            } else {
                // 普通文件：逐个写入 zip 条目
                try (FileInputStream fis = new FileInputStream(file)) {
                    // 创建 zip 条目（条目名用正斜杠分隔，兼容 Windows/Mac 所有解压软件）
                    zos.putNextEntry(new ZipEntry(entryName));
                    // 4KB 缓冲区，边读边写，避免大文件一次性读入内存
                    byte[] buffer = new byte[4096];
                    int len;
                    // 循环读取源文件内容写入 zip 流，直到读完
